...
$a=(mt_rand(1,187));
$sql = "SELECT * FROM countries WHERE count_index ='$a'";
...
while($row = mysqli_fetch_array($result_1,MYSQLI_ASSOC))
{
   ...
   $pop = $row["count_pop"];
   $flagg = $row["count_fc"];
   $linkk = $row["count_link"];
}
?>
<html>
	<head>
		<link rel="stylesheet" type="text/css" href="css_countt.css">
		<title>HANGMAN</title>
	</head>
<body>
<script src="mousetrap.js"></script>
<button class="button" id="STARTANIMATE" onclick="start();">START</button>
<button class="button" id="RESTART" onclick="restart();">NEXT</button>
<br/>
...
function showinfo()
{
	...
	"<strong>Capital: </strong><?php echo $cap;?>";
	document.getElementById("area").innerHTML = 
	"<strong>Area (in sq. kms.): </strong><?php echo $area;?>";
	document.getElementById("population").innerHTML = 
	"<strong>Population: </strong><?php echo $pop;?>";
	document.getElementById("flag").innerHTML = 
	'<img src="countries/flags/<?php echo $flagg;?>.png">';
	document.getElementById("linkk").innerHTML =
	'<a href="<?php echo $linkk;?>">Wikipedia Link</a>';
}
